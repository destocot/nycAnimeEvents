...
export const GET = async () => {
  return NextResponse.json({ mssg: "turned off" }, { status: 200 });
  await db.eventDate.deleteMany();
  await db.event.deleteMany();
  await db.date.deleteMany();
  ...
  for (const event of events) {
    ...
    for (const eventDate of event.date) {
      await db.eventDate.create({
        data: {
          ...
          event: {
            connect: { eventId: newEvent.eventId },
          },
        },
      });
    }
  ...
  const [{ _count: totalEvents }] = await db.event.findMany({
    select: { _count: true },
  });

  console.log(totalEvents.eventDates);

  const isSuccess = totalEvents.eventDates === events.length;
